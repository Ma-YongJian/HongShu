...
import com.hongshu.web.mapper.sys.SysFileMapper;
import com.hongshu.web.service.*;
import com.hongshu.web.service.sys.LocalFileService;
import com.hongshu.web.service.sys.MinioService;
import com.hongshu.web.service.sys.QiniuService;
...
@Slf4j
@Service
public class FileServiceImpl extends SuperServiceImpl<SysFileMapper, WebFile> implements IWebFileService {
    ...
    public String getPicture(String fileIds, String code) {
        if (StringUtilss.isEmpty(code)) {
            code = Constantss.SYMBOL_COMMA;
        }
        if (StringUtilss.isEmpty(fileIds)) {
            log.error(MessageConf.PICTURE_UID_IS_NULL);
            return ResultUtil.result(SysConf.ERROR, MessageConf.PICTURE_UID_IS_NULL);
        } else {
            List<Map<String, Object>> list = new ArrayList<>();
            List<String> changeStringToString = StringUtilss.changeStringToString(fileIds, code);
            QueryWrapper<WebFile> queryWrapper = new QueryWrapper<>();
            queryWrapper.in(SQLConf.UID, changeStringToString);
            List<WebFile> fileList = fileService.list(queryWrapper);
            if (fileList.size() > 0) {
                for (WebFile file : fileList) {
                    if (file != null) {
    ...
    public String batchUploadFile(HttpServletRequest request, List<MultipartFile> filedatas, SystemConfig systemConfig) {
        ...
        //projectName现在默认base
        if (StringUtilss.isEmpty(projectName)) {
            projectName = "base";
        }

        //TODO 检测用户上传，如果不是网站的用户就不能调用
        if (StringUtilss.isEmpty(userUid) && StringUtilss.isEmpty(adminUid)) {
            return ResultUtil.result(SysConf.ERROR, "请先注册");
        }

        QueryWrapper<WebFileSort> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(SQLConf.SORT_NAME, sortName);
        queryWrapper.eq(SQLConf.PROJECT_NAME, projectName);
        queryWrapper.eq(SQLConf.STATUS, EStatus.ENABLE);
        List<WebFileSort> fileSorts = fileSortService.list(queryWrapper);

        WebFileSort fileSort = null;
        if (fileSorts.size() >= 1) {
        ...
        List<WebFile> lists = new ArrayList<>();
        //文件上传
    ...
    public Object ckeditorUploadCopyFile() {
        ...
        if (EFilePriority.QI_NIU.equals(systemConfig.getContentPicturePriority())) {
            ...
            if (StringUtilss.isNotEmpty(systemConfig.getQiNiuPictureBaseUrl()) && StringUtilss.isNotEmpty(itemUrl) && itemUrl.indexOf(systemConfig.getQiNiuPictureBaseUrl()) > -1) {
            ...
            // 表示优先显示Minio对象存储
            // 判断需要上传的域名和本机图片域名是否一致，如果一致，那么就不需要重新上传，而是直接返回
            if (StringUtilss.isNotEmpty(systemConfig.getMinioPictureBaseUrl()) && StringUtilss.isNotEmpty(itemUrl) && itemUrl.indexOf(systemConfig.getMinioPictureBaseUrl()) > -1) {
                Map<String, Object> result = new HashMap<>();
                result.put(SysConf.UPLOADED, 1);
                result.put(SysConf.FILE_NAME, itemUrl);
                result.put(SysConf.URL, itemUrl);
            ...
            // 表示优先显示本地服务器
            // 判断需要上传的域名和本机图片域名是否一致，如果一致，那么就不需要重新上传，而是直接返回
            if (StringUtilss.isNotEmpty(systemConfig.getLocalPictureBaseUrl()) && StringUtilss.isNotEmpty(itemUrl) && itemUrl.indexOf(systemConfig.getLocalPictureBaseUrl()) > -1) {
                Map<String, Object> result = new HashMap<>();
                result.put(SysConf.UPLOADED, 1);
                result.put(SysConf.FILE_NAME, itemUrl);
                result.put(SysConf.URL, itemUrl);
        ...
        file.setFileOldName(itemUrl);
        file.setFileSize(0L);
        file.setPicExpandedName(Constantss.FILE_SUFFIX_JPG);
        file.setPicName(newFileName);
        // 设置本地图片
